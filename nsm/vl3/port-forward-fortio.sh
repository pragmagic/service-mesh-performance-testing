...
pods=$(k2 -n msm-perf-test-msm get pods -l app=fortio -o go-template --template="{{range .items}}{{.metadata.name}} {{end}}") || exit
...
for pod in $pods
do
    k2 -n msm-perf-test-msm port-forward pods/"$pod" "$port":8080 2>&1 | sed -e 's/^/fwd '"$pod $port"': /' &
